...
class SQLConnectionBase extends ConnectionBase {
  ...
  async aggregate(_queryEngine, literal, options) {
    ...
    let queryGenerator  = this.getQueryGenerator();
    let query           = queryEngine.clone().PROJECT(literal);
  ...
  async pluck(_queryEngine, _fields, options) {
    if (options && !Nife.instanceOf(options, 'object'))
    ...
    // TODO: Map to objects if multiple fields
    // also support option to return raw array

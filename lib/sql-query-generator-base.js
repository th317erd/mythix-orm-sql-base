...
class SQLQueryGeneratorBase extends QueryGeneratorBase {
  ...
  generateSelectQueryCondition(queryPart, _value, options) {
    ...
    if (operator === 'EXISTS' || operator === 'NOT EXISTS')
      return `${operator}(${this.generateSelectStatement(value, this.stackAssign(options, { isSubQuery: true, subQueryOperator: operator }))})`;
  ...
  // eslint-disable-next-line no-unused-vars
  generateOrderClause(order, _options) {
    if (!order || !order.size)
      return '';
    ...
    for (let [ fullyQualifiedFieldName, orderScope ] of order) {
      ...
      if (Nife.instanceOf(value, 'string')) {
        ...
        // literal here.
        if (options.projectionFields && !options.projectionFields.has(fullyQualifiedFieldName) && contextOrderSupport === 'PROJECTION_ONLY')
          continue;
  ...
  // eslint-disable-next-line no-unused-vars
  generateGroupByClause(_fields, _options) {
    if (LiteralBase.isLiteral(_fields))
      return _fields.toString(this.connection);

    let fields = Nife.toArray(_fields).filter(Boolean);
    ...
    for (let i = 0, il = fields.length; i < il; i++) {
      ...
      if (LiteralBase.isLiteral(groupField)) {
        let fieldStr = groupField.toString(this.connection);
        if (!options.projectionFields.has(fieldStr) && contextOrderSupport === 'PROJECTION_ONLY')
          continue;

        groupByParts.push(fieldStr);
        continue;
      }

      // Only allow fields that are in our projection
      if (options.projectionFields && options.onlyProjectedFields !== false) {
        let modelName   = groupField.Model.getModelName();
        let fieldName   = groupField.Field.fieldName;
      ...
      let escapedColumnName = this.getEscapedColumnName(groupField.Model, groupField.Field.columnName, options);
      groupByParts.push(escapedColumnName);
    }

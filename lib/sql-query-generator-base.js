...
class SQLQueryGeneratorBase extends QueryGeneratorBase {
  ...
  generateGroupByClause(groupBy, _options) {
  ...
  generateGroupByAndHavingClause(queryEngine, options) {
    ...
    let groupBy = context.groupBy;
    if (!groupBy || !groupBy.size)
      return '';

    let sqlParts          = [];
    let groupByStatement  = this.generateGroupByClause(groupBy, options);
    if (groupByStatement)
      sqlParts.push(groupByStatement);

    let having = context.having;
    if (having) {

...
class SQLQueryGeneratorBase extends QueryGeneratorBase {
  ...
  generateColumnIndexes(Model, field, _indexes, _options) {
    let indexes = Nife.toArray(_indexes).filter((index) => {
      if (index === true)
        return true;
  ...
  generateCreateTableStatementOuterTail(Model, options) {
    ...
    Model.iterateFields(({ field }) => {
      ...
      let result = this.generateColumnIndexes(Model, field, field.index, options);
  ...
  generateAlterTableStatement(Model, newModelAttributes, options) {
    ...
    if (newModelAttributes.tableName) {
